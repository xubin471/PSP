...
from dataloader.datasets import TestDataset
from dataloader.dataset_specifics import *
from utils import *
...
def main(_run, _config, _log):
    ...
    for label_val, label_name in labels.items():
        ...
        with torch.no_grad():
            ...
            support_image = [support_sample['image'][[i]].float().cuda() for i in
                             range(support_sample['image'].shape[0])]  # n_shot x 3 x H x W, support_image is a list {3X(3, 3, 256, 256)}
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                ...
                C_q = sample['image'].shape[1]    # slice number of query init
                ...
                for sub_chunck in range(_config['n_part']):  # n_part = 3
                    ...
                    support_image_s = [support_image[sub_chunck]]  # 3 x 3 x H x W
                    support_fg_mask_s = [support_fg_mask[sub_chunck]]  # 3 x H x W
                    query_image_s = query_image[0][idx_[sub_chunck]:idx_[sub_chunck + 1]]  # C' x 3 x H x W
                    query_label_s = query_label[0][idx_[sub_chunck]:idx_[sub_chunck + 1]]  # 4 x 256 x 256
                    query_pred_s = []
                    for i in range(query_image_s.shape[0]):
                        cur_id += 1
                        _pred_s= model([support_image_s][0][0],  query_image_s[[i]], support_fg_mask_s[0],None,
                                           mode='test')  # 3 x 3 x H x W
                        query_pred_s.append(_pred_s)

                    query_pred_s = torch.cat(query_pred_s, dim=0)  # [9, 3, 256, 256]  [9, 3, 256, 256]
    ...
    with open('trial_data_analyze/trial_result', 'a+') as file:
        file.write(str(class_dice)+"\n")
    ...
    _log.info(f'Total Mean Dice: {value}')
