...
from .ca import CA
from .pos_attention import PriorGuidedSelfAttention
from utils import *
class FewShot(nn.Module):
    def __init__(self):
        ...
        self.pos_attention = PriorGuidedSelfAttention()
        self.CA = CA()
    ...
    def forward(self, sup_img,qry_img, sup_msk,qry_msk,mode="train"):
        ...
        # ================================================================================
        # sup_fts = self.pos_attention(sup_fts.clone(), sup_msk.unsqueeze(1))
        sup_fts = self.self_attention(sup_fts.clone())
        qry_fts = self.self_attention(qry_fts.clone())
        # ================================================================================
        sup_glob_proto , shape_scores, rec_loss_ = self._extract_shape_prototypes(sup_fts,sup_msk)
        ...
        # qry_pred_logit_pos = self.pos_prior(qry_pred_softmax_[:,1:,:,:],sup_msk)
        # qry_pred_logit_pos = qry_pred_softmax_[:, 1:, :, :]

        # qry_pred_softmax_final = torch.cat([1-qry_pred_logit_pos,qry_pred_logit_pos],dim=1)
        # ================================================================================
        if mode == "train":
            ...
            if torch.argmax(qry_pred_softmax_final,dim=1).sum() > 200:
                bd_shape_loss += 0.005 * self.loss.bd_shape_loss(qry_pred_softmax_final[:,1:,:,:],sup_msk.unsqueeze(1))
    ...
    def align_loss(self, sup_fts, qry_fts, sup_msk, qry_pred,shape_scores1):
        ...
        # ================================================================================
        # qry_fts = self.pos_attention(qry_fts.clone(), qry_pred_mask.unsqueeze(1))
        sup_fts = self.self_attention(sup_fts.clone())
        qry_fts = self.self_attention(qry_fts.clone())

        # qry_fts, sup_fts = self.pos_attention(qry_fts.clone(),qry_pred_mask.unsqueeze(1)), self.pos_attention(sup_fts.clone(),qry_pred_mask.unsqueeze(1))

        qry_glob_proto, shape_scores, rec_loss_ = self._extract_shape_prototypes(qry_fts,qry_pred_mask)
        sup_fts = sup_fts * shape_scores[...,None,None]
        # align_loss += rec_loss_
        # ================================================================================
        sup_pred_softmax = self.single_proto_predict(sup_fts,qry_glob_proto,self.sup_t,sup_msk.shape[-2:]) #[1 1 256 256]
        # align_loss += self.loss.coarse_loss(sup_pred_softmax,sup_msk.long())
        # =================================================================================
        ...
        # sup_pred_logit_pos = self.pos_prior(sup_pred_softmax_[:,1:,:,:],qry_pred_mask)
        # sup_pred_softmax_final = torch.cat([1-sup_pred_logit_pos,sup_pred_logit_pos],dim=1)
        # =================================================================================
